...
app.get("/api/health", (_req, res) => {
  // serverTimeMs lets the client measure its wall-clock offset, so two
  // laptops honour the same playAtUnixMs (see apps/web/src/rooms/timeSync.ts).
  res.json({
    ok: true,
 ...
 * ──────────────────────────────────────────────────────────────────────────
 */
app.post("/api/turns/:roomId/score", (req, res) => {
  ...
  // LANE B: verdict merge / Atlas write / ELO goes here, before recordScore.
  const settled = recordScore(io, room, player.id, score as ScoreCard);
...
io.on("connection", (socket) => {
  socket.on(ClientEvents.playerHello, (payload: unknown) => {
    ...
    const session: Session = {
      ...
      displayName:
        typeof displayName === "string" && displayName.trim()
          ...
          : "Singer",
      // LANE B's Atlas layer is the source of truth for ELO; until it lands
      // everyone starts at the default.
      elo: StartingElo,
      roomCode: null,
    };
